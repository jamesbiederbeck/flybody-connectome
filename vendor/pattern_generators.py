"""Wing beat pattern generator, vendored verbatim from TuragaLab/flybody.

Source: flybody/tasks/pattern_generators.py @ d015e9b (Apache-2.0).
Only change: the two constants it imported from flybody/tasks/constants.py are
inlined below, so this module depends on numpy alone and neither flybody nor
dm_control needs installing.

Why a pattern generator at all: *Drosophila* is an asynchronous flier.  The
DLM/DVM power muscles are stretch-activated and oscillate at the thorax's
mechanical resonance (~200 Hz) while their motor neurons fire roughly an order
of magnitude slower, so the per-beat rhythm is genuinely not neurally patterned.
Driving wing position from per-spike DLM activity would model the fly as a
synchronous flier and be *less* faithful, not more.  What is a kludge is the
specific form -- prescribed kinematics replayed at a commanded frequency rather
than a simulated resonance -- and with `base_pattern_path=None` it is a bare
sine approximation that upstream's own docstring calls "not a substitute for a
realistic base wing pattern".  See README "Accepted approximations".
"""

import numpy as np

# --- inlined from flybody/tasks/constants.py -------------------------------
_FLY_CONTROL_TIMESTEP = 2e-4  # s
_WING_PARAMS = {
    'base_freq': 218.,
    'gainprm': [18, 18, 18],
    'damping': 0.007769230,
